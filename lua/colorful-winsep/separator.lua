...
function Separator:horizontal_init(width, highlight_start, highlight_end, title)
  ...
  local start_s, body_s, end_s = self.start_symbol, self.body_symbol, self.end_symbol
  local start_w, end_w = vim.fn.strwidth(start_s), vim.fn.strwidth(end_s)
  local body_w = width - start_w - end_w

  local line_content
  local title_start_byte, title_end_byte = -1, -1

  local header_cfg = config.opts.header
  if title and title ~= "" and header_cfg and header_cfg.enabled then
    ...
    if title_w <= body_w then
      ...
      local left_body = string.rep(body_s, left_pad_w)
      local right_body = string.rep(body_s, right_pad_w)

      line_content = start_s .. left_body .. display_title .. right_body .. end_s
      title_start_byte = #start_s + #left_body
      title_end_byte = title_start_byte + #display_title
    end
  end

  if not line_content then
    line_content = start_s .. string.rep(body_s, body_w) .. end_s
  end
  ...
  if title_start_byte ~= -1 then
    api.nvim_buf_set_extmark(self.buffer, ns_id, 0, title_start_byte, {
      ...
      hl_group = header_cfg.highlight or "ColorfulWinSepHeader",
    })
  end

  -- Highlight end
  if highlight_end ~= false then
    api.nvim_buf_set_extmark(self.buffer, ns_id, 0, #line_content - #end_s, {
